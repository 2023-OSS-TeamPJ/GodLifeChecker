...
void saveData(standard *s,daily *d[]  ,int count){
    ...
    fprintf(fp,"%s %s %s %s %s %s %s \n",s->exerciseTime, s->majorStudy, s->minorStudy,s->sleepTime, s->eatingCount,s->readingTime,s->friendshipTime );
    fclose(fp);

    fp = fopen("DailyData.txt", "wt");
    for(; i < count ; i ++){
        if(s[i]== NULL)continue;
        fprintf(fp,"%s %s %s %s %s %s %s %s \n", d[i]->dailycount, d[i]->exerciseTime,d[i]->majorStudy, d[i]->minorStudy,d[i]->sleepTime, d[i]->eatingCount,d[i]->readingTime,d[i]->friendshipTime);
    }
    fclose(fp);
    printf("SAVE!!\n");
...
int loadData(standard *s, daily *d[]){
    ...
    for(; i < 100; i++){
        ...
        fscanf(fp,"%d",&s->minorStudy);
        fscanf(fp,"%d",&s->sleepTime);
        fscanf(fp,"%d",&s->eatingCount);
    ...
    for(; i < 100; i++){
        ...
        fscanf(fp,"%d",&d[i]->dailycount);
        if(feof(fp))break;
        fscanf(fp,"%d",&d[i]->exerciseTime);
        fscanf(fp,"%d",&d[i]->majorStudy);
        fscanf(fp,"%d",&d[i]->minorStudy);
        fscanf(fp,"%d",&d[i]->sleepTime);
        fscanf(fp,"%d",&d[i]->eatingCount);
